...
for run in sys.argv[1:]:
    ...
    print(
        " ".join(
            f"{y}: {sum(r_at(t, 4) for t in g) / len(g):+.2f}/{sum(r_at(t, 4) - t['cost_r'] for t in g) / len(g):+.2f} ({len(g)})"
    ...
    for lo, hi in zip(edges, edges[1:]):
        ...
        if g:
            print(
                f"  {lo:.2f}-{hi:.2f}: N={len(g)} gross {st([r_at(t, 4) for t in g])} net {st([r_at(t, 4) - t['cost_r'] for t in g])}"
            )
